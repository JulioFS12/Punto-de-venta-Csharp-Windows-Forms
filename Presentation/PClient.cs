...
{
    public partial class PClient : Form
    {
        public PClient()
        {
            InitializeComponent();
                ...
                if (Opcion == DialogResult.OK)
                {
                    ...
                    foreach (DataGridViewRow row in dataGridViewCategory.Rows)
                    {
                        ...
                        if (Convert.ToBoolean(row.Cells[0].Value))
                        {
                            ...
                            Rpta = LClient.Eliminar(Convert.ToInt32(Codigo));
        ...
        {
            PViewClient form = PViewClient.GetInstancia();
            AddOwnedForm(form);
            string par1, par2,  par5, par6, par7, par8;
            par1 = Convert.ToString(this.dataGridViewCategory.CurrentRow.Cells["Id Cliente"].Value);
            par2 = Convert.ToString(this.dataGridViewCategory.CurrentRow.Cells["Nombre y Apellido"].Value);
            par5 = Convert.ToString(this.dataGridViewCategory.CurrentRow.Cells["Tipo de documento"].Value);
            par6 = Convert.ToString(this.dataGridViewCategory.CurrentRow.Cells["Numero de documento"].Value);
            par7 = Convert.ToString(this.dataGridViewCategory.CurrentRow.Cells["Direccion"].Value);
            par8 = Convert.ToString(this.dataGridViewCategory.CurrentRow.Cells["Telefono"].Value);
            form.setCliente(par1, par2, par5, par6, par7, par8);
            form.ShowDialog();
        }
        ...
        private void PClient_Load(object sender, EventArgs e)
        {
            this.ShowTable();
        }
